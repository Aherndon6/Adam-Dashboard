...
**Test baselines live in State, never in Law (rule added 2026-09-12).** Law previously
carried `Static regression tests: 1332/0` and `E2E: 130/0`. Both rotted silently and
were badly wrong: the static suite ran **1788/14** on `main` from 2026-08-08, and e2e
had been red since **2026-07-11** — for two months — while this file still claimed
green. A pass/fail count changes on every commit, so it is operating state, not a
stable rule, and a stale number here is worse than no number: it tells a session the
suite is green when it is not, so nobody investigates.

Do NOT reintroduce a test count into this file. The current verified baseline belongs
in `CODEX_STATUS.md`, recorded with the date and commit it was measured at. Before
