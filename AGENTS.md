...
5F-1.5 Gate A (Wendy July usability) UI shipped and live (2026-07-05/06): A5 account-dropdown alphabetization, A8 weekly banner in header, A6 sortable Register columns, A9a Register search/type/status filters, A9b Register date filters, A7a read-only Category Report modal + picker, A7b Budget expense-row drill-through, plus the Register Quicken-style ledger hotfix (Date desc default, historical/as-of-transaction-date Balance, starting-balance row at bottom, Clr status-only with uncleared review via the Status filter). Display/read-only scope except the pre-existing Clr checkbox behavior, which was preserved unchanged; no new Register write path, schema/RLS/RPC, or Budget-calculation changes.
Static regression tests: 1322/0 passing.
...
- A4 (AMEX Gold starting-balance correction) is PARKED. `accounts.starting_balance` for AMEX Gold is wrong (8248.07) and should be 8248.50 as-of 2026-07-01. This is a Register-ledger-display-only value (does not affect Budget spend, cashflow, or reconciliation). The hardened data-fix SQL (`docs/2026-07-05-amex-gold-starting-balance.sql`, untracked, unexecuted, excluded from the pushed stack) is blocked by its own baseline guard: a 2026-06-30 Foxtail -$7.17 AMEX Gold transaction predates the 2026-07-01 anchor. Needs Wendy/accounting confirmation whether 8248.50 is pre- or post-Foxtail before running.
