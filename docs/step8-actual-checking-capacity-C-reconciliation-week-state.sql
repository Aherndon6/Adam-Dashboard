...
--     additionally required for C3 (per-week checking-ledger coverage).
...
--   client-side. Snapshot columns are COUNTS of the nine eligible goal ids and of
--   total rows — never funded_amount values. Deterministic by week_num.
------------------------------------------------------------------------------
...
recon AS (SELECT week_num, (chk IS NOT NULL) AS reconciled FROM public.weekly_reconciliations),
snap AS (
  SELECT gfs.week_num,
...
SELECT 'C2_WEEK_STATE_MATRIX' AS result_set,
  w.week_num,
  coalesce(r.reconciled, false)                         AS reconciled,
  ...
  CASE
    ...
    WHEN NOT coalesce(r.reconciled,false) AND coalesce(s.distinct_eligible,0)=0 THEN 'open'
    WHEN coalesce(r.reconciled,false) AND coalesce(s.distinct_eligible,0) >= 9  THEN 'complete_db_derived'
    WHEN coalesce(s.distinct_eligible,0) BETWEEN 1 AND 8 THEN 'half_closed_db_derived'
    ELSE 'indeterminate_review'
...
--   Requires A4 gate B in addition to gate C. For each model week 1..31 this
...
--   transaction_date falls in that span, alongside the reconciled flag. It exposes
--   week-level ledger coverage (e.g. reconciled weeks with zero checking activity,
--   or activity in unreconciled weeks) WITHOUT any amount. The span is INFERRED,
--   NOT stored — every downstream use must carry that caveat (U2). Balance-free;
--   counts only. Deterministic by week_num.
------------------------------------------------------------------------------
WITH weeks AS (
...
recon AS (SELECT week_num FROM public.weekly_reconciliations WHERE chk IS NOT NULL)
SELECT 'C3_LEDGER_COVERAGE_BY_WEEK' AS result_set,
  w.week_num,
  w.week_start_inferred,
  w.week_end_inferred,
  EXISTS (SELECT 1 FROM recon rc WHERE rc.week_num = w.week_num) AS reconciled,
...
--   value, its expectation, and a verdict (PASS | REVIEW | STOP). The final
--   C4.OVERALL row rolls them up (STOP dominates REVIEW dominates PASS). Counts
--   and booleans only — balance-free. Deterministic by check_id (OVERALL sorts last).
------------------------------------------------------------------------------
WITH eligible(goal_id) AS (
  VALUES ('adam_ira'),('wendy_ira'),('wendy_sep'),('alaska'),('bailey_529'),
         ('bryce_529'),('preston_529'),('bryce_vehicle'),('christmas_cruise')
),
wr AS (SELECT week_num, balance_basis, recorded_at, (chk IS NOT NULL) AS reconciled FROM public.weekly_reconciliations),
recd AS (SELECT week_num, balance_basis, recorded_at FROM wr WHERE reconciled),
snap AS (
...
recset AS (SELECT count(*) AS n, min(week_num) AS mn, max(week_num) AS mx FROM recd),
inversions AS (
  SELECT count(*) AS n FROM (
    SELECT recorded_at, lag(recorded_at) OVER (ORDER BY week_num) AS prev FROM recd
  ) q WHERE prev IS NOT NULL AND recorded_at < prev
)
SELECT 'C4_RECONCILIATION_PREREQUISITES' AS result_set, 'C4.1' AS check_id,
...
UNION ALL
SELECT 'C4_RECONCILIATION_PREREQUISITES','C4.2',
...
UNION ALL
SELECT 'C4_RECONCILIATION_PREREQUISITES','C4.3',
  'Anchor week (model week 5) is reconciled',
  (SELECT count(*) FROM recd WHERE week_num = 5)::text, '1',
  CASE WHEN (SELECT count(*) FROM recd WHERE week_num = 5)=1 THEN 'PASS' ELSE 'REVIEW' END
UNION ALL
SELECT 'C4_RECONCILIATION_PREREQUISITES','C4.4',
  'Reconciled weeks form a contiguous prefix 1..N (no holes)',
  (SELECT (n = mx AND mn = 1) FROM recset)::text, 'true',
  CASE WHEN (SELECT (n = mx AND mn = 1) FROM recset) THEN 'PASS' ELSE 'REVIEW' END
UNION ALL
SELECT 'C4_RECONCILIATION_PREREQUISITES','C4.5',
  'Reconciled weeks carry a balance_basis (non-NULL)',
  (SELECT count(*) FROM recd WHERE balance_basis IS NULL)::text, '0 (legacy pre-anchor may legitimately be NULL)',
  CASE WHEN (SELECT count(*) FROM recd WHERE balance_basis IS NULL)=0 THEN 'PASS' ELSE 'REVIEW' END
UNION ALL
SELECT 'C4_RECONCILIATION_PREREQUISITES','C4.6',
  'Post-anchor reconciled weeks (>=6) have full closeout snapshots (9 distinct eligible)',
  (SELECT count(*) FROM recd r WHERE r.week_num >= 6
     AND coalesce((SELECT distinct_eligible FROM snap s WHERE s.week_num = r.week_num),0) < 9)::text, '0',
  CASE WHEN (SELECT count(*) FROM recd r WHERE r.week_num >= 6
     AND coalesce((SELECT distinct_eligible FROM snap s WHERE s.week_num = r.week_num),0) < 9)=0 THEN 'PASS' ELSE 'REVIEW' END
UNION ALL
SELECT 'C4_RECONCILIATION_PREREQUISITES','C4.7',
  'No closeout snapshots on unreconciled post-anchor weeks (>=6)',
  (SELECT count(*) FROM snap s WHERE s.week_num >= 6 AND NOT EXISTS (SELECT 1 FROM recd r WHERE r.week_num = s.week_num))::text, '0',
  CASE WHEN (SELECT count(*) FROM snap s WHERE s.week_num >= 6 AND NOT EXISTS (SELECT 1 FROM recd r WHERE r.week_num = s.week_num))=0 THEN 'PASS' ELSE 'REVIEW' END
UNION ALL
SELECT 'C4_RECONCILIATION_PREREQUISITES','C4.8',
...
UNION ALL
SELECT 'C4_RECONCILIATION_PREREQUISITES','C4.9',
  'recorded_at is non-decreasing across ascending reconciled week_num',
  (SELECT n FROM inversions)::text, '0 (informational)',
  CASE WHEN (SELECT n FROM inversions)=0 THEN 'PASS' ELSE 'REVIEW' END
UNION ALL
SELECT 'C4_RECONCILIATION_PREREQUISITES','C4.OVERALL',
  'Overall reconciliation-prerequisite gate (STOP dominates REVIEW dominates PASS)',
  NULL, 'C_PREREQ_PASS',
  CASE
    WHEN (SELECT count(*) FROM dupes) > 0
      ...
      THEN 'C_PREREQ_STOP'
    WHEN (SELECT count(*) FROM recd WHERE week_num = 5) <> 1
      OR NOT coalesce((SELECT (n = mx AND mn = 1) FROM recset), false)
      OR (SELECT count(*) FROM recd WHERE balance_basis IS NULL) > 0
      OR (SELECT count(*) FROM recd r WHERE r.week_num >= 6
            AND coalesce((SELECT distinct_eligible FROM snap s WHERE s.week_num = r.week_num),0) < 9) > 0
      OR (SELECT count(*) FROM snap s WHERE s.week_num >= 6 AND NOT EXISTS (SELECT 1 FROM recd r WHERE r.week_num = s.week_num)) > 0
      OR (SELECT n FROM inversions) > 0
