...
- **DEFECT (5G-1B rider — stable executed-transfer identity and history preservation)** (logged 2026-07-11, OPEN, docs-only). After Week-5 reconciliation, executed model transfer rows across **Alaska, Wewe RCCL, Wewe DCL, and both Adam IRA actions** disappear from "Transfers to execute": rows are re-derived from `runModel`, and completion is keyed positionally by `(week_num, task_idx)` in `public.weekly_tasks` — `action_key`/`completed_label` persist but are orphaned and overwrite-vulnerable. No data/reconciliation/financial loss; Week-5 recon valid; **not an E2 blocker**. Immediate correction owned by **5G-1B**, but the identity/render adapter applies to **all** model-generated transfers; cross-refs 5G-1D (durable funded attribution), 5G-1E (AMEX holding integrity), reconcile→re-render test coverage. Post-E2 fix candidate. Full record: `docs/5g-1b-defect-reconciled-transfer-history-2026-07-11.md`.
...
| 5G-1B | Holding→payout lifecycle for RCCL/DCL cruise deposits (release event) | Deferred | Not started (known accepted AMEX-Savings offset until then); relates to 5G-1E. **Carries the 5G-1B-rider reconciled-transfer-history defect (2026-07-11) — stable executed-transfer identity/history, applies to all model-generated transfers; see `docs/5g-1b-defect-reconciled-transfer-history-2026-07-11.md`; post-E2 fix candidate.** |
