...
| 5G-1C-2 | Goal Funding State Integrity (`goal_funding_snapshots` + overwrite-at-anchor overlay + RPC + first opening_anchor + golden-master) | Pre-Alaska if freeze checkpoint met | **E1 prod DDL COMPLETE + GREEN (2026-07-09)** — schema-only/EMPTY, immutable; C3 overlay shipped + inert. Remaining step is the **E2 first-anchor seed — NOT RUN / BLOCKING (this is Gate 0 for 5G-1D).** See CODEX_STATUS "## 5G-1C-2 E1 COMPLETE" + "## 5G-1D READINESS CLEARED". |
| 5G-1D | Weekly Closeout Goal Attribution (prefilled one-click snapshot write at closeout, calls the 5G-1C-2 RPC) | Post-1C-2 | **Planning CLEARED** (plan `6de4614` + correction/reopen/remediation companion spec `f005263` + implementation-readiness package `a55d899`, all on main). **Implementation NOT STARTED — BLOCKED BY GATE 0 (E2 not executed).** See "#### 5G-1D readiness handoff" below + CODEX_STATUS "## 5G-1D READINESS CLEARED". |
