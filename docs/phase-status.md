...
> **✅ 5G-1D PRODUCTION ACTIVATION COMPLETE + VERIFIED (2026-07-18).** 5G-1D is production-live: Gate-B
> executed on `usayoldrawwmjsmretin` (Phase-1 grants → deploy `BUILD_TS 2026-07-17T23:10:13`/`bdcd1d7`
> → wk6 wrapper closeout → Phase-2 revokes → Proof A/B → Option B (durable Week-6 Adam IRA snapshot
> corrected to the approved annual target)). Gate C
> lockdown active; old direct write paths revoked; Week 6 remains reconciled and immutable (owner-only
> reopen control) with the activation execution window closed. Any "staging-accepted but not
> production-activated / Gate B OPEN / not production-live" language below is **historical** — see the
...
> applied on the feature branch — **Adam-verified at commit `114b080`: static 1507/0, full `node
> e2e.js` 148/0/0, readiness fallbacks openApp 0 / clickNav 0.** **Still
...
- **TX-1 — Transaction Category Integrity + Income Taxonomy + Budget Attribution** (future candidate, not sequenced, NOT UX-0.5). Register data-quality workstream: source-accurate BKCPA Extra Pay income category (`income.bkcpa_extra_pay` / `commission_income` / `display_only`), reimbursement/offset categories, required-category + save validation for manual transactions, uncategorized review/filter + cleanup list, and budget-period attribution/carryover ("covered by June" vs current-month Extra). Coordinates with the 5F-3 month-boundary/carryover backlog item but is distinct (funding-period of a correctly-dated, correctly-categorized charge). No Weekly Model cash-math changes unless explicitly required; no duplicate inflow/tax rule/goal allocation. Full definition + three motivating examples: `docs/tx-1-candidate.md`. **Post-activation sub-item TX-1.1 (2026-07-17):** register income category labeled **exactly** `Wendy Extra BK Pay` (weekly-model match) + recategorize the 2026-07-17 BK $1,752.26 inflow; post-activation only — see `docs/tx-1-candidate.md` §TX-1.1.
- **DEFECT (5G-1B rider — stable executed-transfer identity and history preservation) — RESOLVED 2026-07-11 (commit `db2704f`; static 1431/0, e2e 135/0, smoke 19/0, browser clean).** (logged 2026-07-11, docs-only record). After Week-5 reconciliation, executed model transfer rows across **Alaska, Wewe RCCL, Wewe DCL, and both Adam IRA actions** disappeared from "Transfers to execute": rows are re-derived from `runModel`, and completion is keyed positionally by `(week_num, task_idx)` in `public.weekly_tasks` — `action_key`/`completed_label` persist but are orphaned and overwrite-vulnerable. No data/reconciliation/financial loss; Week-5 recon valid; **not an E2 blocker**. Immediate correction owned by **5G-1B**, but the identity/render adapter applies to **all** model-generated transfers; cross-refs 5G-1D (durable funded attribution), 5G-1E (AMEX holding integrity), reconcile→re-render test coverage. Post-E2 fix candidate. Full record: `docs/5g-1b-defect-reconciled-transfer-history-2026-07-11.md`.
...
#### Authoritative 5G sub-phase roadmap (adopted 2026-07-08 — supersedes the 2026-07-06 map below)

Per Adam decision (2026-07-08), the funding-model integrity review (`docs/funding-model-integrity-review-2026-07-08.md`) reorganized 5G so the **goal-funding-integrity track (5G-1A … 5G-1F) comes first**, then Planned Outflows, Cash Allocation, and goal-management/close hardening. This is now the authoritative forward sequence:

| Sub | Name | Timing | Status |
|-----|------|--------|--------|
| 5G-0 | Roadmap/label cleanup ("Available for Goals" rename); no logic change | Pre-Alaska | Complete (2026-07-07); static 1332/0, e2e 131/0 |
| 5G-1A | Weekly Transfer Routing + Readiness (RCCL/DCL → AMEX Savings holding) | Pre-Alaska | **Shipped** (2026-07-07, `c8613bc`); static 1339/0, e2e 131/0 |
| 5G-1A.5 | AMEX Hold Sub-`MIN_XFR` Deadlock Hotfix | Pre-Alaska | **Shipped + pushed** (2026-07-08, `f307db7`, prod-verified); static 1359/0 |
| 5G-1B | Holding→payout lifecycle for RCCL/DCL cruise deposits (release event) | Deferred | Not started (known accepted AMEX-Savings offset until then); relates to 5G-1E. **5G-1B-rider reconciled-transfer-history defect RESOLVED 2026-07-11 (commit `db2704f`; adapter/UI only; static 1431/0, e2e 135/0, smoke 19/0) — see `docs/5g-1b-defect-reconciled-transfer-history-2026-07-11.md`. Holding→payout lifecycle itself remains deferred.** |
| 5G-1C-1 | Funding Plan Projection Semantics (display-only "When"-column labels) | Pre-Alaska | **Shipped + pushed** (2026-07-08, `de4e3c0`, prod-verified); static 1374/0, e2e 132/0 |
| 5G-1C-2 | Goal Funding State Integrity (`goal_funding_snapshots` + overwrite-at-anchor overlay + RPC + first opening_anchor + golden-master) | Pre-Alaska if freeze checkpoint met | **E1 prod DDL COMPLETE + GREEN (2026-07-09)** — schema-only/EMPTY, immutable; C3 overlay shipped. **E2 first-anchor seed COMPLETE + GREEN (2026-07-11)** — 9 opening_anchor rows at wk 5, SA-PROD PASS, SA8 expected under-attribution (no over-attribution), C3 overlay now live; **Gate 0 for 5G-1D now SATISFIED.** See CODEX_STATUS "## 5G-1C-2 E2 COMPLETE" + closeout `docs/phase-5g-1c-2-e2-closeout-2026-07-11.md`. **5G-1C-2.1 (post-anchor coherence hotfix) COMPLETE + GREEN (2026-07-11)** — Leg 1 code (`c0a3476`) + Leg 2 two guarded Week-5 holding-state `correction` snapshots (`wewe_rccl=600`/`wewe_dcl=500`, executed once → 11 wk-5 rows, nine opening_anchor rows proven unchanged; correction run once, never rerun; templates unfilled, no household values); see CODEX_STATUS "## 5G-1C-2.1 COMPLETE" + `docs/phase-5g-1c-2.1-hotfix.md`. |
| 5G-1D | Weekly Closeout Goal Attribution (two-step atomic combined closeout via `save_weekly_closeout_with_snapshots`) | Post-1C-2 | **✅ PRODUCTION-LIVE + COMPLETE + VERIFIED (2026-07-18)** — Gate B executed on `usayoldrawwmjsmretin`; Phase-2 revokes active; Proof A (idempotent) + Proof B (legacy RPC 403/42501) PASSED; Option B — Adam IRA durable Week-6 snapshot corrected to the approved annual target and verified; Week 6 reconciled + immutable (owner-only reopen control), activation execution window closed; closeout `docs/phase-5g-1d-activation-closeout-2026-07-18.md`. *(Development history below is retained; any "not production-live / Gate B OPEN" phrasing is superseded by this marker.)* **RPC layer staging-accepted** (Slices 1/2, Gate-2 matrix 2026-07-13). **Browser Slices 3/4/5 COMPLETE (2026-07-13)** — confirmation view + wrapper wiring + state machine + closeout-state badges + half-close repair; branch-held on `claude/herndon-5g-1d-preactivation-j428vn` (pushed, NOT merged — `main` auto-deploys and the wrapper is ungranted until Gate B). Independent pre-production corrections P0-1…P2 + final-review items F1–F3 applied (incl. the row-9 `deleteRecon` guard + closeout response validation). **Adam-verified at commit `114b080`: static 1507/0, full `node e2e.js` 148/0/0, readiness fallbacks openApp 0 / clickNav 0** (supersedes 142/0 & 1486/0); `BUILD_TS` unchanged. **Gate D DECIDED: Option A pre-freeze (Adam, 2026-07-13).** Pre-activation planning DELIVERED (Gate C register + activation grant SQL + Slice 6 runbook + Gate B runbook, all 2026-07-13, NOT executed). **Slice 6 inert production deployment COMPLETE + GREEN (2026-07-13)** — wrapper + Option B live in production Adam-Dashboard (`usayoldrawwmjsmretin`), INERT (zero API-role EXECUTE; bodies byte-unchanged; DR restore point captured); all 5 gates passed; see `docs/phase-5g-1d-slice6-closeout-2026-07-13.md`. **(HISTORICAL — SUPERSEDED by the 2026-07-18 completion marker above):** Gate C dispositions approved and **executed at Gate B (2026-07-18)**; **Gate B activation COMPLETE**; Gate E OPEN-when-triggered (never triggered). Writes subsequent weekly closeout snapshots only — the Week-5 `opening_anchor` remains E2's. **Model week 6 = calendar Week 28 (`getCalWeek(6)=28`); the first supervised closeout persists `week_num=6`. POST-ACTIVATION CONSTRAINT (Fable P2-2): `weekly_reconciliations` is keyed only by `week_num` (no `model_year`) — before the first 2027 closeout it must be re-keyed (add `model_year`), archived, or otherwise handled to avoid a 2026/2027 week-number collision.** See CODEX_STATUS "## 5G-1D BROWSER COMPLETE + GATE D DECIDED" + `docs/phase-5g-1d-gatec-register-2026-07-13.md` / `-slice6-deploy-runbook-2026-07-13.md` / `-gateb-activation-runbook-2026-07-13.md` / `-gateb-adjunct-preflight.sql`. |
| 5G-1E | Account Purpose / Holding Bucket Integrity (promotes AMEX invariant advisory→hard; consumes 5G-1B releases) | Post-1D | Not started |
| 5G-1F | July Month-End Close / Audit Minimum Viable Close | Post-1E | Not started |
| 5G-2 | Planned Outflows Foundation (`planned_outflows` + `outflow_events`; Mint seed) — **the DB/security layer was already staging-validated under the legacy "5G-1" label (see below)** | Post-1F | DB/security layer validated on staging (2026-07-08, `eeee4cb`/`7f0d0a0`); prod DDL + app build not started |
| 5G-3 | Cash Allocation / Spoken For / Free to Use (derived view; no stored allocation balances) | Post-2 | Not started |
| 5G-4 | Dynamic Goal Management Architecture | Post-3 | Not started |
| 5G-5 | Goal Admin UI | Post-4 | Not started |
| 5G-6 | Full Month-End Close / Audit Hardening | Post-5 | Not started |
...
**Legacy technical gates — carry-forward mapping (RESOLVED 2026-07-14; see the "Legacy fold-in resolution" table below):** the superseded 2026-07-06 map below carried specific technical gates that must NOT be lost — **calc-core extraction / characterization under golden-master (legacy 5G-2.5)**, the **set-aside AMEX-lookahead / max-safe-sweep gate and derived-not-stored deferred set-asides (legacy 5G-4a)**, the **zero-outflow identity committed test and earmark adapter into the 5F-1 engine (legacy 5G-4b)**, the **Budget-identity change / Available-for-Goals-becomes-derived work (legacy 5G-3)**, and **legacy-system / spreadsheet retirement (legacy 5G-5)**. **Correction (2026-07-13, Adam-confirmed):** the legacy-5G-5 trigger "after one clean parallel month" is superseded — Quicken parallel operation has ended, the subscription is canceled, and the OS is the sole live system of record (see the Operating-state note at the top of this file). **No additional clean parallel month is required.** Any remaining legacy-5G-5 work is now scoped to archival verification, retained-data/retention policy, export verification, documentation cleanup, and removal of obsolete workflow references — with the retained Quicken historical data protected (archive/reference only; not to be deleted or modified without explicit Adam approval). The precise fold-in (which new 5G-2…5G-6 phase each legacy gate attaches to, and whether the Budget-identity change is absorbed or re-scheduled) is **RESOLVED in the "Legacy fold-in resolution" table below** (2026-07-14; authority `docs/roadmap/canonical-roadmap.md` §9). The superseded 2026-07-06 map is retained below for its legacy technical-gate detail only.

#### Legacy fold-in resolution (RESOLVED 2026-07-14 — canonical roadmap §9; prepared under A6)

The open fold-in item above is **resolved**. Canonical vocabulary and phase attachment (authority: `docs/roadmap/canonical-roadmap.md` §9; the calc-core amendment is folded there):

| Legacy gate | Resolves to | Canonical home |
|---|---|---|
| **5G-2.5** — calc-core extraction / characterization under golden-master | **Calc-Core Extraction** (named phase; "5G-2.5" retired except as legacy provenance) | Canonical **P6** |
| **legacy 5G-3** — Budget-identity change / Available-for-Goals becomes derived | **Budget-Identity Change (legacy 5G-3)** — always written with the "legacy" qualifier | Canonical **P8-adjacent** (gated on L2 + A2 income actuals + Wendy inputs + extraction) |
| **legacy 5G-4a** — set-aside AMEX-lookahead / max-safe-sweep gate; derived-not-stored set-asides | gate language folds into the **5G-2 spec** (transfer-recommendation control); derived-not-stored rule into the **5G-3 spec** | 5G-2 / 5G-3 specs |
| **legacy 5G-4b** — zero-outflow identity committed test; earmark adapter into the 5F-1 engine (input layer only) | **new 5G-4b** (goal write capability); identity test capturable at extraction | Canonical **P6 (5G-4b)** |
| **legacy 5G-5** — spreadsheet / Quicken retirement | residual = **D1 archival policy + docs cleanup**; **no phase** (the "one clean parallel month" trigger is void — Quicken retired) | D1 (decision register) |

**Naming rule:** new-numbering ordinals only for product phases; named phases (Calc-Core Extraction, Budget-Identity Change, Monthly Close v1) carry no ordinal; legacy ordinals appear only as parenthetical provenance. "Planning engine" is retired — it means the calculation core. Do not introduce "5G-2.5" or "planning engine" into forward-looking text.

#### 5G-1D readiness handoff (2026-07-10)

**SUPERSEDED IN PART (2026-07-11): E2 is COMPLETE + GREEN; Gate 0 SATISFIED — see the 5G-1C-2 row above + `docs/phase-5g-1c-2-e2-closeout-2026-07-11.md`. The "BLOCKED BY GATE 0 / OPEN/BLOCKING / E2 unexecuted" language in this dated 2026-07-10 handoff is historical.** The 5G-1D planning stack is **CLEARED and on `main`**; **5G-1D implementation has NOT started and is BLOCKED BY GATE 0 (E2 not executed).** This sub-block is the phase-map pointer; the visible summary (decisions D1–D3, the Gate 0/A–E summary register, sequencing, and non-authorization language) lives in **CODEX_STATUS.md "## 5G-1D READINESS CLEARED — E2 / GATE 0 BLOCKING (2026-07-10)"**, and the **authoritative full gate register + slice plan** is `docs/phase-5g-1d-implementation-readiness-2026-07-10.md` (`a55d899`).

- **Governing artifacts:** E2 runbook `docs/phase-5g-1c-2-e2-runbook.md` (`c439d68`); 5G-1D plan `docs/phase-5g-1d-plan-2026-07-09.md` (`6de4614`); correction/reopen/remediation companion spec `docs/phase-5g-1d-snapshot-correction-procedure-2026-07-10.md` (`f005263`); implementation-readiness package `docs/phase-5g-1d-implementation-readiness-2026-07-10.md` (`a55d899`).
- **Settled decisions:** D1 Option A is a temporary bridge only; D2 Option B (owner-only correction wrapper, `is_owner()`, Wendy-rejected, existing-row + nearest-existing bounds, call-through, no anchor amendments) is built in Slice 2; D3 Week-5 anchor amendments preserve `source='opening_anchor'`.
- **Blocking gate:** Gate 0 (E2 completion) is OPEN/BLOCKING before Slice 0; Gates A–E (is_owner() identity, Option B activation, repair-RPC posture, activation timing, multi-week remediation) are OPEN at their triggers — see the readiness package §3.
- **Non-authorization:** recording these cleared artifacts does not authorize implementation; E2 remains unexecuted; every future implementation/deployment/activation/correction/reopen/remediation/grant/rollback/production action requires its own gate + approval. **No 5G merge during the Alaska freeze (Jul 29–Aug 10; corrected 2026-07-14 — see canonical roadmap §11.3).**

#### Superseded detailed map (2026-07-06) — retained for legacy technical-gate detail only
...
| 5G-1 | planned_outflows + outflow_events schema; seed Mint Mobile; append-only Set Aside / Paid / Adjust; opening adjustment from dated snapshot on posted balances; Mint rows transfer_funded to AMEX Savings; auto_renew=false | Pre-Alaska | Staging DB/security layer validated (rehearsal + RLS smoke + `app_environment` hardening, 2026-07-08; `eeee4cb`/`7f0d0a0`); prod DDL + app build not started — **now tracked as new 5G-2** |
| 5G-2 | Derived Account Allocation view (Spoken For / Free to Use); no stored allocation balances | Pre-Alaska | Not started — **now tracked as new 5G-3** |
| 5G-2.5 | Calculation Core Extraction / Characterization under golden-master tests; no framework, no UI refactor, no build step | Post-Alaska | Not started — **legacy gate; fold-in TBD (see carry-forward note)** |
...
**5G-1A.5 shipped + pushed (2026-07-08, commit `f307db7`, production-verified): AMEX Hold Sub-MIN_XFR Deadlock Hotfix.** Phase A of the 2026-07-08 funding-model integrity review (`docs/funding-model-integrity-review-2026-07-08.md`); runModel freeze exception approved by Adam. An `_amxHold` goal left with a sub-`MIN_XFR` (<$100) remainder passed through `maxSafeAmxSweep()` (which floors any amount below MIN_XFR to 0) before `mv()`; the 0 triggered defer+`break`, permanently starving every lower-priority goal (Adam IRA stuck 7438.94/7500 = 99% after the `1dcc686` IRA correction; Wendy IRA + all 529s $0 from Cal Wk 28, Bailey "Beyond 2026"). Fix: completion carve-out at the `_amxHold` call site mirroring the existing `mv()` `allowFin` rule, gated by the existing 5-week `amxSweepKeepsFloor` check (floor-unsafe sub-$100 sweeps still defer); defer-label accuracy (below-$100-minimum vs genuine floor risk); `retRem` sourced from the registry Adam IRA target ($7,500), not hardcoded 7000. runModel freeze exception approved. `index.html` + `test_regression.js` only; no schema/RLS/RPC/SQL, no priority/target/reconciliation changes; Week 27 transfer outputs byte-identical. Static **1359/0**, e2e **131/0**; Adam live-smoked the local build (Adam IRA completes Cal Wk 29, Wendy IRA continuation, Week 27 unchanged, floor-risk label preserved). Bailey 529 reaching 73% with the stale "Beyond 2026" row label is a live-surplus/projection-semantics property → resolved by **5G-1C-1** (below), not a blocker. Full detail in CODEX_STATUS.md "## 5G-1A.5 SHIPPED".

**5G-1C-1 shipped + pushed (2026-07-08, commit `de4e3c0`, production-verified): Funding Plan Projection Semantics.** Slice 1 of the two-slice 5G-1C plan (`docs/phase-5g-1c-plan-2026-07-08.md`; Fable review `docs/phase-5g-1c-plan-review-2026-07-08.md`, "5G-1C-1: GO now"). Display-only Funding Plan "When"-column labels via a new pure `_fundingWhenLabel()` helper that distinguishes current-week funded from projected year-end funded (read-only off the final model week's `goalSaved`): retires the misleading "Beyond 2026" ($0-now goals with projected partial funding now read "Partial in 2026 · Continues in 2027" + a Projected-YE line; locked-but-accumulating goals show their projected completion week; fully-funded+locked shows "Staged — awaiting CPA clearance"). No runModel/waterfall/getGoalFunded/goalCompletion/balances/schema/RPC/RLS changes; PHASE-A Week-27 golden stayed green. Static **1374/0**, e2e **132/0**. Full detail in CODEX_STATUS.md "## 5G-1C-1 SHIPPED". **5G-1C-2 (Goal Funding State Integrity — `goal_funding_snapshots`) remains HOLD** pending the plan doc update with Fable R1–R13.

**Phase-map reconciliation — EXECUTED (Adam approved, 2026-07-08; D5):** the new roadmap is authoritative and is now reflected in the "Authoritative 5G sub-phase roadmap" table above — 5G-1A…1F funding-integrity track first, then 5G-2 Planned Outflows Foundation / 5G-3 Cash Allocation / 5G-4 Dynamic Goal Management / 5G-5 Goal Admin UI / 5G-6 Full Month-End Close. Legacy "5G-1" (planned_outflows, staging-validated) → new 5G-2; legacy "5G-2" (Derived Account Allocation) → new 5G-3. Docs-only; no code/SQL/schema/RPC/RLS change. **Fold-in RESOLVED 2026-07-14 (canonical roadmap §9):** the legacy technical gates (calc-core extraction → **Calc-Core Extraction**; set-aside AMEX-lookahead legacy-5G-4a; zero-outflow identity test legacy-5G-4b; Budget-identity change legacy-5G-3; spreadsheet retirement legacy-5G-5) are folded into the new numbering per the "Legacy fold-in resolution" table above.
...
**Pre / post-Alaska:** 5G-0 through 5G-2 are pre-Alaska candidates. Only 5G-0 is safe to start before a staging Supabase exists; 5G-1 requires a staging Supabase plus a baseline export via `scripts/export-ai-review-pack.sh` (both now in place; the 5G-1 staging DB schema + RLS were validated 2026-07-08 and torn down clean). Calc-Core Extraction (legacy 5G-2.5), 5G-3, 5G-4a, 5G-4b, 5G-5 are post-Alaska unless explicitly pulled forward after review. 5G-3+ also gates on Wendy feedback. Freeze window: **July 29 through August 10** (corrected 2026-07-14; no routine main pushes — two named exception classes per canonical roadmap §11.3).

**Gates:** *(phase numbers below are LEGACY 2026-07-06 numbering; they carry forward but their attachment to the new 5G-2…5G-6 phases is the open fold-in item noted in the roadmap section above — do not read "5G-3/5G-4a/5G-4b" here as the new roadmap's phases.)*

- Before legacy-5G-3 (Budget-identity change): Wendy response on Budget / Available for Goals workflow; A2 income-actuals sequencing; calculation-core extraction (legacy-5G-2.5) complete.
- Before legacy-5G-4a (set-aside recommendations): Diablos/GLP WD gap fixed as a separate task; WC-3 disposition resolved or formally quarantined; fresh baseline weekly-model output captured after the WD fix; safe-sweep gate inside the extracted calculation/recommendation layer and characterization-covered.
- Before legacy-5G-4b (earmark adapter): zero-outflow identity gate exists as an automated committed test; reserve/commitment precedence rule added to the closeout checklist (if an actual payment enters reconciliation as a Phase 3 manual commitment in a due week, the outflow release is recorded in the same closeout so reserve and commitment never overlap).
