...
- **P3b-1 Data Integrity** — TX-1 taxonomy (income/offset categories) + required-category + save validation + uncategorized cleanup; `budget_line_rules.category_key`→`categories` FK audit (**AGENTS.md Near-Term Wishlist ID 39** / `docs/validation-blr-category-sync.sql` / `docs/2026-07-02-register-budget-category-sync.sql` — **not** live `wishlist_items` ID 39, which is mobile usability → 5H; see Appendix B.3); reimbursement-status home decision (Register status columns vs manual side-channel). Includes **TX-1.1** — the register income category must be labeled **exactly** `Wendy Extra BK Pay` (weekly-model match, drift guard; `income.bkcpa_extra_pay` / `commission_income` / `display_only`) and recategorize the 2026-07-17 BK $1,752.26 inflow (No Category → `Wendy Extra BK Pay`); **post-activation only**, register-classification/reporting only, no tax-rule/goal duplication (`docs/tx-1-candidate.md` §TX-1.1). *Feeds close quality; sheds last.*
  ...
  - **5G-1D-HIST-1 — Reconciled-week legacy-completion display + write-lock [post-5G-1D; before/with Monthly Close v1].** UI / non-actionable treatment for reconciled weeks that **never fabricates row identity**: disable check **and** uncheck controls + custom-task toggles on reconciled weeks; render unmatched historical recommendations neutral/non-actionable; correct/annotate inflated historical open counts; **preserve open-week fail-closed** behavior (a legacy/null row must never falsely satisfy an open obligation). **No positional fallback, no backfill of null-metadata completions.** Tests: reconciled legacy rows, open-week null-key rows, properly-keyed current-week rows. *Exposed by the `4ce6aff` identity-strict resolver during the 5G-1D activation sitting (Fable Gate-3 disposition 2026-07-18, PROCEED WITH CONDITIONS); durable reconciliation/snapshot data unaffected. Source: operator package §13; decision-log 2026-07-18.*
  - **B1 — Commission-Tax Obligation Netting [PRE-FREEZE URGENT; NOT part of P3b].** A narrowly scoped adapter / candidate-render / Edit-Week-replacement / write-guard remediation of the commission-tax obligation display + write path (distinct from 5G-1B, from the `4ce6aff` B1–B4 findings, and from the operator-package activation step "B1"). One shared pure netting fn `remaining = max(0, total − Σ executed)`; identity by `(action_key=commission_tax, origin ct-override week, (week_num,task_idx))`, never labels; fail-closed controls; write-guard refuses a mismatched amount (e.g. `365.32`) and persists the authoritative `417.83`; Edit-Week drops the delta-PATCH (**retires §2d**). **Must NOT** touch `runModel`/frozen calc core, add schema/migrations, create a durable obligation-lifecycle object or tax `cash_commitments`, merge into `computeGoalTransferNetting`, alter `weekly_reconciliations.tax` semantics, or solve P2-2 re-keying (deferred to **5G-1B / P2-2 / Calc-Core Extraction**). The `runModel` surplus-derived projection skew (bound `52.51`, self-heals at next anchor) stays frozen → **Calc-Core Extraction**. **Timing: target Jul 23 / preferred ≤ Jul 25 / hard pre-freeze cutoff Jul 28** (operational need before the Week-7 / Cal Wk 29 closeout); if it slips, execute the supervised `417.83` under existing controls, keep the never-answer rule + §2d, resume Aug 11+ — **no freeze exception assumed.** Adjacent **D-DISP** display item (Funded-to-date vs Projected-by-year-end) may co-deploy, separate code path. *Fable REVISE → PROCEED 2026-07-18. Authoritative spec: `docs/phase-5g-1d-b1-commission-tax-netting-design-2026-07-18.md`; decision-log 2026-07-18; supersedes operator package §14.6 D5.*
