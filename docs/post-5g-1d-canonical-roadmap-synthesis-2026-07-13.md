...
- **Calc-core extraction** (re-slotted legacy 5G-2.5). Objective: end freeze-exception accretion; give the rollover a place to change constants and 5G-4 a testable waterfall API. Scope: reify the four seams (eligible set / capacity / allocation policy / decision emission); decide skip-vs-break deliberately under golden masters; externalize constants (OP_FL, MIN_XFR, lookahead depth, seed thresholds); **migrate the 1,400+ source-pattern tests to behavioral fixtures.** Hard deps: golden masters (exist); an approved test-migration plan. Frozen? **Yes** — move-only under golden-master identity; this is the phase that *un-freezes* runModel per spec. **Review? Yes** — the seam design + skip-vs-break decision benefit from an external architecture review.
