...
## CURRENCY NOTE (2026-07-18) — ✅ 5G-1D PRODUCTION ACTIVATION COMPLETE + VERIFIED (supersedes all "activation UNAUTHORIZED / not production-live" language below)

- **5G-1D is PRODUCTION-LIVE and COMPLETE.** The full Gate-B sequence executed and verified on production `usayoldrawwmjsmretin`, one owner-gated step at a time (Claude ran no SQL/RPC; Adam executed, Claude verified): Phase-1 grants → Gate-2 deploy (`BUILD_TS 2026-07-17T23:10:13`, prod app `bdcd1d7`) → Week-28 / model-wk-6 closeout via the atomic `save_weekly_closeout_with_snapshots` wrapper (1 recon + 9 `source=reconciliation` snaps; Gate-3 income-collision resolved Option-C/§2d PATH-B\*; AMEX via Phase-3 manual recon) → **Phase-2 revokes** (`LOCKDOWN REVOKES PASS`; old recon/repair/snapshot RPC + table writes revoked for `authenticated`; SELECT retained) → final grant validation (consolidated `post_phase_2` **17/17**).
- **Post-revoke proofs PASSED.** **Proof A** wrapper idempotency (HTTP 200; `ok/mode=normal_closeout/idempotent/week_num=6/snapshot_count=9`; pre/post fingerprints unchanged). **Proof B** legacy direct-RPC denial (`save_reconciliation_with_commitments` HTTP 403 / code 42501; no 2xx; fingerprints unchanged).
- **Option B remediation COMPLETE + VERIFIED.** Gate-4 short-snapshot fix via `correct_goal_funding_snapshot` (owner-authenticated): **the durable Week-6 Adam IRA snapshot was corrected to the approved annual target and verified through Option B** → HTTP 200 `ok=true`/`corrected=true`; post-verify D6 8+1 partition (`recon_source=8`, `correction_source=1`, `surprise_source=0`, `recon_fp` unchanged, `other8_fp=ddcce19d…`). App: the Week-29 duplicate Adam-IRA recommendation is GONE; the Week-28 executed Adam-IRA transfer history is intact; Adam IRA shows fully funded; Wendy IRA is next. **Week 6 remains reconciled and immutable** under the approved owner-only reopen control; the §7 activation execution window is closed (freeze held byte-identical through both proofs; single Option B correction applied; no further wk6 mutation — Week 6 is not open, unfrozen, or reopened). Exact amounts live in the dated evidence records (`docs/phase-5g-1d-activation-closeout-2026-07-18.md`, decision-log).
- **Closeout evidence:** `docs/phase-5g-1d-activation-closeout-2026-07-18.md`; execution index rows in `docs/execution-ledger.md` (2026-07-18); governance row in `docs/decision-log.md`.
- **PENDING future work (distinct — NOT part of this completed phase):** B1 correction code before the Week-7 closeout (§14.6 D5); **push of the local docs commits (owner-authorized only — not done)**; `weekly_reconciliations` 2026/2027 re-key before the first 2027 closeout (Fable P2-2); `weekly_reconciliations` anon grant normalization (operator package §12.5); 5G-1D-HIST-1 legacy-completion display regression (roadmap P3b-1); TX-1.1 income category for `Wendy Extra BK Pay`; Week-29 two-leg tax (amounts in the operator package / decision-log); and the **NEW Stage-2 "2026 Goal-Funding Projection and Waterfall Integrity Audit"** (`docs/phase-audit-2026-goal-funding-waterfall-integrity-2026-07-18.md`) triggered by the post-correction Funding Plan now projecting **Bailey 529** to 100% in 2026 — read-only, separate phase.
- **Everything below this note predates activation** and is retained as history; where older sections read "activation UNAUTHORIZED", "not production-live", or "Gate B OPEN", that language is historical and corrected here.
