...
## CURRENCY NOTE (2026-07-29b) — Baseline E rev-6.1a remediation COMPLETE; final Fable re-verification = APPROVE WITH NON-BLOCKING CONDITIONS; first LOCAL commit authorized (no push, no live execution)

- **rev-6.1a blocker remediation complete.** The four targeted Fable delta-review blockers are closed: N-1 (old fixture UUID fully removed — all forms; negatives derive from the documented synthetic `2e6f8777-…`), N-2 (B5 semantic date validation — Feb-30/rollover/invalid-component/noncanonical-tz rejected; no bare `Date.parse`), N-3 (candgen-v1 manifest deep exact-equality against the frozen closed manifest), N-4 (README stub/`todo` language removed). Related cleanups done (unreachable H2 codes removed; missing `evidence_root` fail-closed; `lp('')` rejected per spec §1; no digest-version/preimage revision).
- **Final independent Fable re-verification verdict: `APPROVE WITH NON-BLOCKING CONDITIONS`** (commit-readiness only). **150/150 tests pass** (0 fail/skipped/todo); **all 17 targeted mutations caught at their intended controls** (M1/M6/M7/M8/M9/M10/M11/M19/M23 + H-1/H-2/H-3/zero-manifestation/coverage-dedup/casefold→toLowerCase/legacy-over-allocation). Pinned Unicode 15.1.0 source SHA-256 `4e55acfd…372bcf` + generated-table `e0afcc45…7e727f` verified unchanged; repository byte-identical across the read-only review. **No commit blockers remain.**
- **Authorization scope:** this covers the **first LOCAL commit only** (`baseline-E/` + `docs/baseline-e-identity-predicate-rev-6.1.md` + this `CODEX_STATUS.md`, `--no-verify` solely to keep the BUILD_TS hook off `index.html`). **Push remains separately owner-gated.** **Live execution remains UNAUTHORIZED** — the before-live-execution gates (adapter differential-oracle, parity harness vs `runModel`, RLS independent-visibility proof, Supabase auth-subject UUID-version confirmation, fresh live-bank Baseline F snapshot + reconciliation) are all still pending. **No capacity calculation, no Wendy-IRA conclusion, no transfer. Operational HOLD remains active.**

## CURRENCY NOTE (2026-07-29a) — Baseline E identity-predicate rev-6.1 FROZEN; local impl exists; Fable Mode-2 = REVISE BEFORE COMMIT → remediated; NO commit authorized

- **Baseline E identity-matching predicate**: rev-6.1 specification is **FROZEN and owner-accepted (2026-07-29)** at `docs/baseline-e-identity-predicate-rev-6.1.md`. The executable implementation exists **locally and untracked** under `baseline-E/` (dependency-free ESM + `node:test`); the docs-only Mode-1 design approval commit is `a48f5ac`.
- **Independent Fable Mode-2 review returned `REVISE BEFORE COMMIT`.** A remediation pass has been completed locally: H-1 authority binding (fail-closed), H-2 graph-identity↔edge reconciliation, H-3 closed anchor-type vocabulary, isolated §10 residual<0 twin, the previously-surviving mutation twins (M1/M6/M7/M8/M9/M10/M11/M19/M23 — all now caught), routing precedence, exactly-once reverse manifestation, coverage de-dup + fixed non-overridable coverage-v1, fail-closed retained-horizon recall, semantic UTC + B5 + candgen-manifest + required-field + negative-zero hardening, legacy `allocation.mjs` over-allocation guard (item K), and the synthetic UUID-v4 fixture replacement.
- **Case folding (owner Option-1, 2026-07-29):** Unicode **15.1.0 FULL** case folding — statuses **C and F included; S and T excluded** — via a repository-pinned generated table. One authorized read-only fetch of `unicode.org/Public/15.1.0/ucd/CaseFolding.txt` (source SHA-256 `4e55acfd…372bcf`, 84,870 bytes) → `baseline-E/data/CaseFolding-15.1.0.txt`; generated table SHA-256 `e0afcc45…7e727f`. No other network access occurred; no production/staging/Supabase contact.
- **Local test status:** 150/150 pass; targeted mutation checks confirm every previously-surviving mutation is now caught (scratch-copy verified).
- **Boundaries:** **no commit authorized** (awaiting owner review of the remediation report); **nothing staged/committed/pushed**; **no live Baseline E execution, no checking-capacity calculation, no Wendy-IRA result, no transfer, no SQL, no production/staging access**; operational **HOLD remains active**. Recommended next: a **targeted Fable delta review** of the remediation before any commit authorization. This does not change the Baseline D/E production posture below.

## CURRENCY NOTE (2026-07-28g) — Step 8 Baseline D EXECUTED read-only (D0–D9 PASS, D_HANDOFF_PASS); next = design Baseline E

- **Active execution pointer:** **Design Baseline E (Actual Checking Capacity synthesis & Wendy-IRA safety test) and assemble its inputs; obtain an independent Fable review before any production execution or transfer decision** (owner-run; **Claude runs no SQL**). **Baseline D→E handoff satisfied.** Do NOT calculate checking capacity or authorize any transfer. **Step 8 is NOT complete.**
- **Baseline D EXECUTED read-only in production `usayoldrawwmjsmretin`, statement-by-statement, 2026-07-28 (owner-run; Claude ran no SQL; NO production mutation).** Frozen artifact `docs/step8-actual-checking-capacity-D-obligations-inflows.sql`, SHA-256 `6de0f8e554cb1a961f362229fa61d27ffa102a2d8240e4604aaeb1de75d8b893` (658 lines). **D0–D9 all PASS; D9 `handoff_gate=D_HANDOFF_PASS`** (as_of_model_week=8, as_of_is_reconciled=false; all seven blocking counters=0: malformed/dup_eid/bkx_outside/engine_reserved_overlap/uncleared_malformed/uncleared_overlap/au11_present; uncleared_valid_candidates=1 informational).
- **Authoritative Baseline D findings:** week 8 unreconciled → projected engine branch; **authoritative DB reserved-obligation total = $0.00** (6 commitments, all reflected_into_balance); one informational uncleared **+$15.00** Bailey valid_candidate (live-bank staleness check required before E consumes it); Alaska **+$770.95** goal_disbursement already recorded/`posted_cleared`/reflected → **NOT additive**; the $435.63 commission-tax commitment reflected/resolved wk5, non-engine-reserved, excluded from D7, does not block D9; **BKX evidence only in custom_tasks** (none in cash_commitments/weekly_tasks); **production remains pre-AU-11** (`au11_objects_present=0`). D8 item-70 static wording ("Alaska not yet recorded") **superseded by executed D2** — frozen SQL preserved as historical evidence, not rewritten.
- **Capacity NOT calculated; HOLD active; no transfer authorized.** Baseline E design is next; Baseline E, Baseline F live-bank snapshot, the final capacity calc, and final validation remain PENDING; a fresh actual-balance + live-bank reconciliation is required before any capacity result. Engine + Wendy IRA + discretionary-transfer holds IN FORCE. Documentation-only update; production untouched by Claude; nothing pushed.

## CURRENCY NOTE (2026-07-28f) — Step 8 Baseline D APPROVED / FROZEN (Fable-reviewed rev-3), NOT executed; next = owner-run Baseline D execution

- **Active execution pointer:** **Execute frozen Baseline D manually against production `usayoldrawwmjsmretin`, statement by statement, capture D0–D9 evidence, and STOP for review before designing or executing Baseline E** (owner-run; **Claude runs no SQL**). Baseline D **has NOT been executed**. **Do NOT begin Baseline E.** **Step 8 is NOT complete.**
- **Baseline D — APPROVED / FROZEN (owner-approved 2026-07-28 after independent Fable review APPROVE-WITH-REQUIRED-CHANGES; all required + owner-required changes applied), NOT executed:** `docs/step8-actual-checking-capacity-D-obligations-inflows.sql` — frozen SHA-256 `6de0f8e554cb1a961f362229fa61d27ffa102a2d8240e4604aaeb1de75d8b893` (658 lines; 11 result sets D0/D1/D2/D2B/D3/D4/D5/D6/D7/D8/D9; rev-2 `9d2569c4…df235`, rev-1 `b3d3c97f…ded14c`). Read-only; deterministic; amount-bearing where obligations/inflows require, no account balances. **F1 branch-correct engine reservation; F2/D2B uncleared coverage; F5 broadened BKX/commission-tax overlap → owner_review + EXCLUDED from D7 + D9 fail-closed; deterministic uncleared handoff (valid/overlap/malformed). F7 deferred (immaterial under expected_item_id UNIQUE).**
- **HEADLINE LIMITATION carried:** the JS `WD` base schedule (paychecks, rent, all card payments, Kia, wk15 Alaska draw) is outside SQL visibility (`index.html:909-941`); Baseline D inventories DB overlays only, and hands the base schedule + forward inflows to Baseline E out-of-band.
- **Capacity NOT calculated; no transfer authorized; HOLD preserved.** Baselines E, F, final capacity calc, and final validation remain PENDING. Engine + Wendy IRA + discretionary-transfer holds IN FORCE. Freeze commit is docs-only; production untouched by Claude; nothing pushed.

## CURRENCY NOTE (2026-07-28e) — Step 8 Baseline D DRAFTED (obligations & inflows), pending independent Fable review; NOT frozen/executed

- **Active execution pointer:** **Step 8 — complete the independent (Fable) review of Baseline D, apply required changes, obtain owner approval, freeze, THEN seek authorization to execute** (owner-run; **Claude runs no SQL**). Design/review session only. **Step 8 is NOT complete.**
- **Baseline D — DRAFT rev-3, FABLE-REVIEWED (APPROVE-WITH-REQUIRED-CHANGES), pending owner approval; NOT frozen, NOT executed:** `docs/step8-actual-checking-capacity-D-obligations-inflows.sql` — rev-3 SHA-256 `6de0f8e554cb1a961f362229fa61d27ffa102a2d8240e4604aaeb1de75d8b893` (658 lines; 11 result sets D0/D1/D2/D2B/D3/D4/D5/D6/D7/D8/D9; rev-2 `9d2569c4…df235`; rev-1 `b3d3c97f…ded14c`). Read-only; deterministic; amount-bearing where obligations/inflows require, **no account balances**; grounded via 3 read-only code/schema maps. **Rev-2 applied Fable F1 (branch-correct `is_engine_reserved`) + F2 (D2B uncleared) + F3/F4/F6/F8. Rev-3 (owner-required) fully applies F5 (BKX/commission-tax overlap → owner_review, EXCLUDED from D7, D9 fail-closed) + tightens the uncleared handoff (D2B valid/overlap/malformed classes; D9 fail-closed on overlap/malformed; E consumes the deterministic valid set). F7 deferred (immaterial under the expected_item_id UNIQUE constraint).**
- **Grounded design facts:** authoritative obligation source = `cash_commitments` (5F-1) with `isReservedAsOf` predicate (`index.html:2438-2448`) encoded verbatim; floor `OP_FL=6500` (`:896`); horizon 31 = `WD` length (`:909-941`); cutoff = model-week integer + model_year=2026 (due_date descriptive-only). **HEADLINE LIMITATION:** base modeled inflows/obligations (paychecks, rent, all card payments, Kia, wk15 Alaska draw) are JS-hardcoded (`WD`, `:909-941`) and INVISIBLE to SQL → D inventories DB overlays only; base schedule + forward inflows are out-of-band to E. `custom_tasks` has no amount col (BKX $700.90 in label) and is NOT read by the model (`:3899`).
- **Capacity NOT established; HOLD preserved.** No D result is a capacity number or transfer authorization. Baselines E, F, final capacity calc, final validation remain PENDING. Engine + Wendy IRA + discretionary-transfer holds IN FORCE. Production untouched by Claude; nothing executed/committed/pushed for D yet (draft in working tree, awaiting Fable + owner approval).

## CURRENCY NOTE (2026-07-28d) — Step 8 Baseline C EXECUTED + PASS (read-only, zero mutation); next = design Baseline D

- **Active execution pointer:** **Step 8 — Design Baseline D (obligations & inflows) for Actual Checking Capacity, obtain independent (Fable) review, freeze it, and only then seek authorization to execute it** (owner-run; **Claude runs no SQL**). **Do NOT begin/draft Baseline D until directed.** **Step 8 is NOT complete.**
- **Baseline C EXECUTED + PASS (owner-run, read-only, statement-by-statement, production `usayoldrawwmjsmretin`, 2026-07-28; Claude ran no SQL).** Frozen artifact `docs/step8-actual-checking-capacity-C-reconciliation-week-state.sql` — hash **`8b4169726b81…0996fb`** (330 lines) verified byte-identical before documenting; **C SQL unchanged**. Results: **C0** `C_GATE_PASS` (19/19); **C1** reconciliations weeks 1–7 only, all five balances present, basis wk1–3 NULL(legacy)/wk4–5 posted/wk6–7 available; **C2** wks1–4 legacy_pre_anchor, wk5 anchor (9 eligible/11 total), wks6–7 complete_db_derived (9/9), wks8–31 open_or_blocked — **no half_closed, no corrupt**; **C3** checking_account_present=1, txn counts wk1–3=0/wk4=8/wk5=12/wk6=9/wk7=18, wks8–31=0, no future activity; **C4** `C4.OVERALL = C_PREREQ_PASS` (all 10 gating checks 0 violations; INFO C4.11=3 legacy NULL-basis rows, C4.12=0 inversions). **Production mutation NONE.**
- **Capacity NOT established.** Baseline C is a reconciliation/week-state integrity pass only — **no checking-capacity figure, no transfer authorization**. **Baselines D, E, F, final capacity calculation, and final validation remain PENDING.** **No live-bank snapshot (Baseline F) captured.** Engine hold + Wendy IRA hold + discretionary-transfer deferrals remain IN FORCE; operational result stays **HOLD**.
- **Record:** `docs/step8-actual-checking-capacity-execution-record-2026-07-28.md` (updated: A/B/C all EXECUTED; Baseline C execution-evidence section added; C4.OVERALL=C_PREREQ_PASS; C4.11/C4.12 informational). Production untouched by Claude; no code/schema/data/BUILD_TS change; nothing pushed.

## CURRENCY NOTE (2026-07-28c) — Step 8 Baselines A + B EXECUTED (read-only, zero mutation; B pass-w/-exceptions); Baseline C APPROVED / FROZEN (Fable-reviewed), NOT executed

- **Active execution pointer:** **Step 8 — Execute Baseline C (Reconciliation & Week State) under the controlled owner-run, read-only production procedure** (owner-run; **Claude runs no SQL**). Baseline C is **APPROVED / FROZEN** at `8b4169726b81…` but **NOT executed** until the owner runs it. **Step 8 is NOT complete.**
- **Baselines A + B EXECUTED against production `usayoldrawwmjsmretin`** by the owner under the controlled read-only procedure. Every statement is SELECT / read-only metadata inspection; **production mutation status = NONE**; **Claude executed no SQL**. **Baseline B = PASS WITH DOCUMENTED OWNER-REVIEWED EXCEPTIONS** (July-1 anchor overlap w/ $2,000 rent; one uncleared +$15 Bailey inflow; two B5_PRIMARY duplicate candidates owner-resolved legitimate; B5_FULL_HISTORY 0 rows; B6a 0 populated pairs; B6b 15 single-candidate/0 ambiguous; B7a/B7b 0 drift; B8 one PAYMENT_LIKE_BUT_POSITIVE owner-resolved expected). **A/B execution parameters ADOPTED:** cutoff_ts=2026-07-28 18:00:00-04:00, cutoff_business_date=2026-07-28, inspection_start_date=2026-06-30. **No live-bank snapshot (Baseline F) captured** → final synchronized capacity cutoff NOT finalized. **No A/B/C result is checking capacity or transfer authorization**; live bank snapshot = primary truth; operational result stays **HOLD**.
- **Baseline C — APPROVED / FROZEN (Fable-reviewed APPROVE-WITH-REQUIRED-CHANGES, all findings applied; owner-approved 2026-07-28), NOT executed:** `docs/step8-actual-checking-capacity-C-reconciliation-week-state.sql` — frozen SHA-256 `8b4169726b81b7b63185734399b969f5b83a4d965a8728ed07f5a3a3460996fb` (330 lines; pre-review draft `59240e54…aed5db`; C0 schema self-preflight, C1 reconciliation inventory, C2 week-state matrix, C3 ledger coverage by week, C4 reconciliation prerequisites). Read-only; **balance-free**; no DDL/DML/RPC; one result set per block; depends on A4 gate C (+ gate B & B1a=TARGET_OK for C3) + its own C0 gate; AU-11 global stop + manual project confirmation apply. **Fable F1/F2 (required) + F3–F8 (hardening) all applied:** C2 closeout CASE made total/app-faithful incl. `corrupt_db_derived`; `reconciled`=row-existence (deployed predicate); C4.5 scoped post-anchor + legacy NULL basis → INFO; recorded_at monotonicity → INFO; C4.4 `count(DISTINCT)`; new C4.9 (NULL-chk) + C4.10 (pre-anchor snapshots); C3 `checking_account_present` guard.
- **Grounded constants (code-cited):** model-week domain 1..31; anchor boundary week 5 (`index.html:4041`); nine snapshot-eligible goals (`index.html:3760`); closeout-complete = reconciled ∧ ≥9 eligible snapshots (`index.html:2166`); reconciled = weekly_reconciliations ROW EXISTS (`isWeekReconciled` `index.html:1043`); week→date span JS-inferred from epoch 2026-06-07 (`index.html:3441`), NOT stored (C3 labels it INFERRED); `goal_funding_snapshots` model_year-scoped (C filters 2026), `weekly_reconciliations` week_num-unique with no model_year.
- **Record:** `docs/step8-actual-checking-capacity-execution-record-2026-07-28.md` (updated: A/B EXECUTED [B pass-w/-exceptions + execution-evidence section], C APPROVED / FROZEN per Fable review, NOT executed). **D, E, F, final capacity calculation, and final validation remain PENDING.** Engine hold + Wendy IRA hold + discretionary-transfer deferrals remain IN FORCE. Production untouched by Claude; no code/schema/data/BUILD_TS change.
...
- **Step 8 = Post-BKX Stabilization "recalculate actual checking capacity"**, namespace **`step8-actual-checking-capacity`**. This is **distinct from** the canonical **AU-11 / 6C-D4** lifecycle work (authenticated end-to-end reservation lifecycle / client routing / disposition / role matrix), which is **untouched**.
- **Two read-only production baseline artifacts stored + frozen (design APPROVED via independent review):**
  - `docs/step8-actual-checking-capacity-A-environment-schema-safety.sql` — SHA-256 `43e97e8c048c76473da561d3977b026859d3b6f3e96fb5d97cb0b609e9a13b9b` (167 lines; A0 session metadata, A1 tables, A2 columns, A3 AU-11 absence [signature-qualified `to_regprocedure`], A4 phased gates B/B6/BCAT/C/D).
  - `docs/step8-actual-checking-capacity-B-checking-register-state.sql` — SHA-256 `df9962ceb10ea3c16b8b70d55a0d1a27e9f173750b99c77bc98abcca515796da` (367 lines; B1a target gate … B8 sign reference classes).
  - Execution record: `docs/step8-actual-checking-capacity-execution-record-2026-07-28.md`.
- **Status pins:** A/B design **APPROVED / FROZEN**; execution **NOT EXECUTED**; production mutation **NONE**; actual cutoff **NOT YET SET** (SQL cutoff values are illustrative placeholders); operational result **HOLD**.
- **Binding notes:** **No A/B result is checking capacity or transfer authorization** (full-DB aggregate is the complete DB-derived ledger calc, subject to anchor validation, duplicate/transfer review, cutoff integrity, and reconciliation to the live bank snapshot = primary actual-balance truth). Target = production `usayoldrawwmjsmretin` (read-only); staging + AU-11 D1/D2/D3 objects out of scope (A3 confirms absence). **C, D, E, F, the final capacity calculation, and final validation remain PENDING/NOT started.** Engine hold + Wendy IRA hold + discretionary-transfer deferrals remain IN FORCE. Production untouched; no BUILD_TS/code/schema/data change.

## CURRENCY NOTE (2026-07-28) — AU-11 Step 6C-D3 composite closeout: STAGING EXECUTION A–I COMPLETE + PASSED (staging-only, dormant)

- **Step 6C-D3 (reservation composite closeout `close_week_with_reservations_v1`) — STAGING EXECUTION A–I COMPLETE + PASSED — STAGING ONLY.** Executed by Adam in the Supabase SQL Editor / two-session psql against staging **`pkwotgqivgaapwuqgwqb`** (≠ prod `usayoldrawwmjsmretin`); **Claude ran no SQL**. One owner-only SECURITY DEFINER 14-arg RPC wrapping the frozen `save_weekly_closeout_with_snapshots`, retiring reservations on cleared Register evidence, plus D-9 durable attribution (`cash_commitments.cleared_transaction_id` + `fk_au11_cleared_txn` + `chk_au11_cleared_txn_attribution` + partial-unique `uix_au11_cleared_txn`) — all dormant, inert grants (anon/authenticated EXECUTE=false), no client wiring/callers. Checkpoints F1 preflight · F2 schema · G1 RPC · G2 catalog-verify · H1 fixture · H2 validation · H3 two-session concurrency · I1 teardown · I2 rollback · I3 post-rollback · I4 clean-reapply · I5 final-state — **all PASSED** (each Code-static + independent-Fable verified before execution). Final staging state (I5, 12/12 PASS): D1+D2+D3 installed & validated; frozen wrapper/recon/snapshot md5 intact (`e2a112b3…`/`1bfde751…`/`154231b3…`); composite RPC exact-14-arg, SECURITY DEFINER, empty search_path, resting-inert; fixture absent + zero residue incl. zero attribution; S3(b) post-attribution invariant holds; exactly one active auth-backed owner. Package: `docs/phase-au11-6c-d3-*` (12 .sql + 4 docs incl. execution record). Frozen surfaces byte-unchanged; no BUILD_TS change.
- **NOT done:** production promotion / D3 production implementation authorization (separate later phase); **production attribution remains NO-GO, additionally gated on D-11 (Register edit/delete protection).** **Engine hold + all operational holds, incl. Wendy IRA, remain IN FORCE.** Production untouched. D3 package authored + staging-validated but **NOT committed/pushed** until owner-authorized.
